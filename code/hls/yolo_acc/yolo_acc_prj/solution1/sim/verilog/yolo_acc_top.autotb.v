...
`define AESL_DEPTH_inStream_a 1
`define AESL_DEPTH_inStream_b 1
...
`define AUTOTB_TVIN_inStream_a  "../tv/cdatafile/c.yolo_acc_top.autotvin_inStream_a.dat"
`define AUTOTB_TVIN_inStream_b  "../tv/cdatafile/c.yolo_acc_top.autotvin_inStream_b.dat"
...
`define AUTOTB_TVIN_inStream_a_out_wrapc  "../tv/rtldatafile/rtl.yolo_acc_top.autotvin_inStream_a.dat"
`define AUTOTB_TVIN_inStream_b_out_wrapc  "../tv/rtldatafile/rtl.yolo_acc_top.autotvin_inStream_b.dat"
...
`define AUTOTB_TVOUT_outStream  "../tv/cdatafile/c.yolo_acc_top.autotvout_outStream.dat"
`define AUTOTB_TVOUT_outStream_out_wrapc  "../tv/rtldatafile/rtl.yolo_acc_top.autotvout_outStream.dat"
module `AUTOTB_TOP;
...
parameter LENGTH_inStream_a = 692224;
parameter LENGTH_inStream_b = 692228;
parameter LENGTH_input_h = 1;
parameter LENGTH_input_w = 1;
parameter LENGTH_leaky = 1;
parameter LENGTH_outStream = 692224;
...
reg [31:0] ap_c_n_tvin_trans_num_inStream_a;
...
reg [31:0] ap_c_n_tvin_trans_num_inStream_b;
...
reg end_inStream_a;
reg [31:0] size_inStream_a;
reg [31:0] size_inStream_a_backup;
reg end_inStream_b;
reg [31:0] size_inStream_b;
reg [31:0] size_inStream_b_backup;
reg end_input_h;
...
begin
    ...
    initial begin : proc_gen_axis_internal_ready_inStream_a
        ...
        forever begin
            @ (ap_c_n_tvin_trans_num_inStream_a or inStream_a_transaction);
            if (ap_c_n_tvin_trans_num_inStream_a > inStream_a_transaction) begin
    ...
    initial begin : proc_gen_axis_internal_ready_inStream_b
        ...
        @ (posedge ready_initial);
        forever begin
            @ (ap_c_n_tvin_trans_num_inStream_b or inStream_b_transaction);
            if (ap_c_n_tvin_trans_num_inStream_b > inStream_b_transaction) begin
    ...
    `define STREAM_SIZE_IN_inStream_a "../tv/stream_size/stream_size_in_inStream_a.dat"
    
    initial begin : gen_ap_c_n_tvin_trans_num_inStream_a
        integer fp_inStream_a;
        reg [127:0] token_inStream_a;
        integer ret;
        
        ap_c_n_tvin_trans_num_inStream_a = 0;
        end_inStream_a = 0;
        wait (AESL_reset === 1);
        
        fp_inStream_a = $fopen(`STREAM_SIZE_IN_inStream_a, "r");
        if(fp_inStream_a == 0) begin
            $display("Failed to open file \"%s\"!", `STREAM_SIZE_IN_inStream_a);
            $finish;
        end
        read_token(fp_inStream_a, token_inStream_a); // should be [[[runtime]]]
        if (token_inStream_a != "[[[runtime]]]") begin
            $display("ERROR: token_inStream_a != \"[[[runtime]]]\"");
            $finish;
        end
        size_inStream_a = 0;
        size_inStream_a_backup = 0;
        while (size_inStream_a == 0 && end_inStream_a == 0) begin
            ap_c_n_tvin_trans_num_inStream_a = ap_c_n_tvin_trans_num_inStream_a + 1;
            read_token(fp_inStream_a, token_inStream_a); // should be [[transaction]] or [[[/runtime]]]
            if (token_inStream_a == "[[transaction]]") begin
                read_token(fp_inStream_a, token_inStream_a); // should be transaction number
                read_token(fp_inStream_a, token_inStream_a); // should be size for hls::stream
                ret = $sscanf(token_inStream_a, "%d", size_inStream_a);
                if (size_inStream_a > 0) begin
                    size_inStream_a_backup = size_inStream_a;
                end
                read_token(fp_inStream_a, token_inStream_a); // should be [[/transaction]]
            end else if (token_inStream_a == "[[[/runtime]]]") begin
                $fclose(fp_inStream_a);
                end_inStream_a = 1;
            end else begin
                $display("ERROR: unknown token_inStream_a");
                $finish;
            end
        end
        forever begin
            @ (posedge AESL_clock);
            if (end_inStream_a == 0) begin
                if ((inStream_a_TREADY & inStream_a_TVALID) == 1) begin
                    if (size_inStream_a > 0) begin
                        size_inStream_a = size_inStream_a - 1;
                        while (size_inStream_a == 0 && end_inStream_a == 0) begin
                            ap_c_n_tvin_trans_num_inStream_a = ap_c_n_tvin_trans_num_inStream_a + 1;
                            read_token(fp_inStream_a, token_inStream_a); // should be [[transaction]] or [[[/runtime]]]
                            if (token_inStream_a == "[[transaction]]") begin
                                ...
                                size_inStream_a = size_inStream_a_backup;
                                $fclose(fp_inStream_a);
                                end_inStream_a = 1;
                            end else begin
                                $display("ERROR: unknown token_inStream_a");
    ...
    `define STREAM_SIZE_IN_inStream_b "../tv/stream_size/stream_size_in_inStream_b.dat"
    
    initial begin : gen_ap_c_n_tvin_trans_num_inStream_b
        integer fp_inStream_b;
        reg [127:0] token_inStream_b;
        integer ret;
        
        ap_c_n_tvin_trans_num_inStream_b = 0;
        end_inStream_b = 0;
        wait (AESL_reset === 1);
        
        fp_inStream_b = $fopen(`STREAM_SIZE_IN_inStream_b, "r");
        if(fp_inStream_b == 0) begin
            $display("Failed to open file \"%s\"!", `STREAM_SIZE_IN_inStream_b);
            $finish;
        end
        read_token(fp_inStream_b, token_inStream_b); // should be [[[runtime]]]
        if (token_inStream_b != "[[[runtime]]]") begin
            $display("ERROR: token_inStream_b != \"[[[runtime]]]\"");
            $finish;
        end
        size_inStream_b = 0;
        size_inStream_b_backup = 0;
        while (size_inStream_b == 0 && end_inStream_b == 0) begin
            ap_c_n_tvin_trans_num_inStream_b = ap_c_n_tvin_trans_num_inStream_b + 1;
            read_token(fp_inStream_b, token_inStream_b); // should be [[transaction]] or [[[/runtime]]]
            if (token_inStream_b == "[[transaction]]") begin
                read_token(fp_inStream_b, token_inStream_b); // should be transaction number
                read_token(fp_inStream_b, token_inStream_b); // should be size for hls::stream
                ret = $sscanf(token_inStream_b, "%d", size_inStream_b);
                if (size_inStream_b > 0) begin
                    size_inStream_b_backup = size_inStream_b;
                end
                read_token(fp_inStream_b, token_inStream_b); // should be [[/transaction]]
            end else if (token_inStream_b == "[[[/runtime]]]") begin
                $fclose(fp_inStream_b);
                end_inStream_b = 1;
            end else begin
                $display("ERROR: unknown token_inStream_b");
                $finish;
            end
        end
        forever begin
            @ (posedge AESL_clock);
            if (end_inStream_b == 0) begin
                if ((inStream_b_TREADY & inStream_b_TVALID) == 1) begin
                    if (size_inStream_b > 0) begin
                        size_inStream_b = size_inStream_b - 1;
                        while (size_inStream_b == 0 && end_inStream_b == 0) begin
                            ap_c_n_tvin_trans_num_inStream_b = ap_c_n_tvin_trans_num_inStream_b + 1;
                            read_token(fp_inStream_b, token_inStream_b); // should be [[transaction]] or [[[/runtime]]]
                            if (token_inStream_b == "[[transaction]]") begin
                                ...
                                size_inStream_b = size_inStream_b_backup;
                                $fclose(fp_inStream_b);
                                end_inStream_b = 1;
                            end else begin
                                $display("ERROR: unknown token_inStream_b");
