...
//------------------------Parameter----------------------
`define TV_IN_activate_en "../tv/cdatafile/c.yolo_yolo_top.autotvin_activate_en.dat"
`define TV_IN_input_h "../tv/cdatafile/c.yolo_yolo_top.autotvin_input_h.dat"
`define TV_IN_input_w "../tv/cdatafile/c.yolo_yolo_top.autotvin_input_w.dat"
parameter ADDR_WIDTH = 6;
parameter DATA_WIDTH = 32;
parameter activate_en_DEPTH = 1;
reg [31 : 0] activate_en_OPERATE_DEPTH = 0;
parameter activate_en_c_bitwidth = 32;
parameter input_h_DEPTH = 1;
reg [31 : 0] input_h_OPERATE_DEPTH = 0;
parameter input_h_c_bitwidth = 5;
parameter input_w_DEPTH = 1;
reg [31 : 0] input_w_OPERATE_DEPTH = 0;
parameter input_w_c_bitwidth = 5;
parameter START_ADDR = 0;
parameter yolo_yolo_top_continue_addr = 0;
parameter yolo_yolo_top_auto_start_addr = 0;
parameter activate_en_data_in_addr = 16;
parameter input_h_data_in_addr = 24;
parameter input_w_data_in_addr = 32;
...
reg  BREADY_reg = 0;
reg [DATA_WIDTH - 1 : 0] mem_activate_en [activate_en_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_activate_en [ (activate_en_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * activate_en_DEPTH -1 : 0] = '{default : 'hz};
reg activate_en_write_data_finish;
...
//write activate_en reg
reg [31 : 0] write_activate_en_count = 0;
reg write_activate_en_run_flag = 0;
reg write_one_activate_en_data_done = 0;
//write input_h reg
...
assign TRAN_CTRL_BUS_write_data_finish = 1 & activate_en_write_data_finish & input_h_write_data_finish & input_w_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        activate_en_write_data_finish <= 0;
        write_activate_en_run_flag <= 0; 
        write_activate_en_count = 0;
        count_operate_depth_by_bitwidth_and_depth (activate_en_c_bitwidth, activate_en_DEPTH, activate_en_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_activate_en_run_flag <= 1; 
            write_activate_en_count = 0;
        end
        if (write_one_activate_en_data_done === 1) begin
            write_activate_en_count = write_activate_en_count + 1;
            if (write_activate_en_count == activate_en_OPERATE_DEPTH) begin
                write_activate_en_run_flag <= 0; 
                activate_en_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_activate_en
    integer write_activate_en_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] activate_en_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = activate_en_c_bitwidth;
    process_num = 1;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_activate_en_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write activate_en data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (activate_en_c_bitwidth < 32) begin
                        activate_en_data_tmp_reg = mem_activate_en[write_activate_en_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < activate_en_c_bitwidth) begin
                                activate_en_data_tmp_reg[j] = mem_activate_en[write_activate_en_count][i*32 + j];
                            end
                            else begin
                                activate_en_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    if(image_mem_activate_en[write_activate_en_count * four_byte_num  + i]!==activate_en_data_tmp_reg) begin
                    write (activate_en_data_in_addr + write_activate_en_count * four_byte_num * 4 + i * 4, activate_en_data_tmp_reg, write_activate_en_resp);
                    image_mem_activate_en[write_activate_en_count * four_byte_num + i]=activate_en_data_tmp_reg;
                    end
                end
                process_busy = 0;
                write_one_activate_en_data_done <= 1;
                @(posedge clk);
                write_one_activate_en_data_done <= 0;
            end   
            process_1_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_activate_en_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [127 : 0] token_tmp; 
  //reg [activate_en_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (activate_en_c_bitwidth , factor);
  fp = $fopen(`TV_IN_activate_en ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_activate_en); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < activate_en_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  mem_activate_en [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
